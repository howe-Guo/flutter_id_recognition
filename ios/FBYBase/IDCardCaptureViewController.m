...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"手电筒" style:UIBarButtonItemStylePlain target:self action:@selector(turnOnOrOffTorch)];
}
...
-(void)addCloseButton {
    ...
    closeBtn.frame = (CGRect){CGRectGetMaxX(viewFrame) - closeBtnWidth - 15.f, CGRectGetMaxY(viewFrame) - closeBtnHeight - 15.f, closeBtnWidth, closeBtnHeight};
...
-(void)showAuthorizationDenied {
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"去设置" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        NSDictionary *options = @{UIApplicationOpenURLOptionUniversalLinksOnly:@true};
        ...
        if (ret <= 0) {
            ...
            if (iDInfo) {// 读取到身份证信息，实例化出IDInfo对象后，截取身份证的有效区域，获取到图像
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [self.navigationController pushViewController:IDCDVC animated:YES];
                });
            }
        }
